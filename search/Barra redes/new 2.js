...
inputBox.onkeyup = (e)=>{
    ...
    if (e.key === 'Enter'){
        if(userData){
            window.open(`https://www.youtube.com/results?search_query=redes+${userData.replace(' ', '+')}`, '_blank');
        }
    }

    if(userData){
        icon.onclick = ()=>{
            webLink = `https://www.youtube.com/results?search_query=redes+${userData.replace(' ', '+')}`;
...
function select(element){
    ...
    icon.onclick = ()=>{
        webLink = `https://www.youtube.com/results?search_query=redes+${selectData.replace(' ', '+')}`;
        linkTag.setAttribute("href", webLink);
        linkTag.click();
    }
    searchWrapper.classList.remove("active");
