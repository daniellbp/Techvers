...
inputBox.onkeyup = (e)=>{
    ...
    let emptyArray = [];
    let itTopic = "tecnologia+da+informação"; // Tema "tecnologia da informação"

    if (e.key === 'Enter'){
        if(userData){
            window.open(`https://www.youtube.com/results?search_query=${itTopic}+${userData.replace(' ', '+')}`, '_blank');
        }
    }

    if(userData){
        icon.onclick = ()=>{
            webLink = `https://www.youtube.com/results?search_query=${itTopic}+${userData.replace(' ', '+')}`;
...
function select(element){
    ...
    icon.onclick = ()=>{
        webLink = `https://www.youtube.com/results?search_query=${itTopic}+${selectData.replace(' ', '+')}`;
        linkTag.setAttribute("href", webLink);
        linkTag.click();
    }
    searchWrapper.classList.remove("active");
