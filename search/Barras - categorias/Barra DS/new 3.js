...
inputBox.onkeyup = (e)=>{
    ...
    if (e.key === 'Enter'){
        if(userData){
            window.open(`https://www.youtube.com/results?search_query=desenvolvimento+de+aplicativos+${userData.replace(' ', '+')}`, '_blank');
        }
    }

    if(userData){
        icon.onclick = ()=>{
            webLink = `https://www.youtube.com/results?search_query=desenvolvimento+de+aplicativos+${userData.replace(' ', '+')}`;
...
function select(element){
    ...
    icon.onclick = ()=>{
        webLink = `https://www.youtube.com/results?search_query=desenvolvimento+de+aplicativos+${selectData.replace(' ', '+')}`;
        linkTag.setAttribute("href", webLink);
        linkTag.click();
    }
    searchWrapper.classList.remove("active");
