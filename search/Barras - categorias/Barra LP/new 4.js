...
inputBox.onkeyup = (e)=>{
    ...
    let emptyArray = [];
    let programmingTopic = "linguagem+de+programação"; // Tema de linguagem de programação

    if (e.key === 'Enter'){
        if(userData){
            window.open(`https://www.youtube.com/results?search_query=${programmingTopic}+${userData.replace(' ', '+')}`, '_blank');
        }
    }

    if(userData){
        icon.onclick = ()=>{
            webLink = `https://www.youtube.com/results?search_query=${programmingTopic}+${userData.replace(' ', '+')}`;
...
function select(element){
    ...
    icon.onclick = ()=>{
        webLink = `https://www.youtube.com/results?search_query=${programmingTopic}+${selectData.replace(' ', '+')}`;
        linkTag.setAttribute("href", webLink);
        linkTag.click();
    }
    searchWrapper.classList.remove("active");
