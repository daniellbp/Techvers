...
inputBox.onkeyup = (e)=>{
    ...
    let emptyArray = [];
    let officeTopic = "pacote+office"; // Tema "pacote office"

    if (e.key === 'Enter'){
        if(userData){
            window.open(`https://www.youtube.com/results?search_query=${officeTopic}+${userData.replace(' ', '+')}`, '_blank');
        }
    }

    if(userData){
        icon.onclick = ()=>{
            webLink = `https://www.youtube.com/results?search_query=${officeTopic}+${userData.replace(' ', '+')}`;
...
function select(element){
    ...
    icon.onclick = ()=>{
        webLink = `https://www.youtube.com/results?search_query=${officeTopic}+${selectData.replace(' ', '+')}`;
        linkTag.setAttribute("href", webLink);
        linkTag.click();
    }
    searchWrapper.classList.remove("active");
